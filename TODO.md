...
## v0.3.0（2026-07-14）
- [x] 生成即时反馈（提交瞬间骨架卡+按钮状态，修"点了像卡住"）
- [x] 输入持久化 localStorage + ✕清空按钮（刷新/切页不丢）
- [x] 发散示例写上界面
- [x] 画风提取宏观化（40-70字，禁具体元素名词——元素多少交给描述管）
- [x] 抽卡页（描述原样×N，建模原画开关默认开；画风+参考图跨页共享，描述独立）
- [x] 出图尺寸选项（方图/横版/竖版）
- [x] 画廊"设为参考图"（可手动做等级进化链）
- [x] 右上角使用教程（7步人话）
- [x] docs/arch.md + docs/ops.md（架构决策+运维手册）
- [ ] 下批：角色设定图模式 → 大纲批量拆解 → A/B 对照
